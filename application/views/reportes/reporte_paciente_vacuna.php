<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">

  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2 justify-content-lg-end">
        <div class="col-sm-1">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="<?php echo base_url() ?>usuario" class="nav-link">Inicio</a></li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
  </section>

  <!-- Main content -->
  <section class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">

          <div class="card">
            <div class="row justify-content-md-center m-3">
              <h3 id="docTitle" ><b>REPORTE DE PACIENTES Y SUS VACUNAS</b></h3>
            </div>
            <div class="card-body">
              <div class="row">
                <!-- ./col -->
                <div class="col-lg-3 col-6">
                  <!-- small card -->
                  <div class="small-box bg-success">
                    <div class="inner">
                      <h3><?php echo $totalHoy ?><sup style="font-size: 20px"></sup></h3>
                      <p id="titulo-hoy">Pacientes vacunados en el dia</p>
                    </div>
                    <div class="icon">
                      <i class="fas fa-check-circle"></i>
                    </div>
                    <a id="reporte-hoy" href="#" class="small-box-footer">
                      Mas informacion <i class="fas fa-arrow-circle-right"></i>
                    </a>
                  </div>
                </div>
                <!-- ./col -->
                <div class="col-lg-3 col-6">
                  <!-- small card -->
                  <div class="small-box bg-warning">
                    <div class="inner">
                      
                      <h3><?php echo $totalPendiente ?></h3>
                      
                      <p id="titulo-vacuna-pendiente">Pacientes con vacunas pendientes</p>
                    </div>
                    <div class="icon">
                      <i class="fas fa-exclamation-triangle"></i>
                    </div>
                    <a id="reporte-pendiente" href="#" class="small-box-footer">
                      Mas informacion <i class="fas fa-arrow-circle-right"></i>
                    </a>
                  </div>
                </div>
                <!-- ./col -->
                <div class="col-lg-3 col-6">
                  <!-- small card -->
                  <div class="small-box bg-danger">
                    <div class="inner">
                      <h3><?php echo $totalRezagado ?></h3>
                      <p id="titulo-vacuna-rezagado">Pacientes que no acudieron a vacunarse</p>
                    </div>
                    <div class="icon">
                      <i class="fas fa-radiation-alt"></i>
                    </div>
                    <a id="reporte-vencido" href="#" class="small-box-footer">
                      Mas informacion <i class="fas fa-arrow-circle-right"></i>
                    </a>
                  </div>
                </div>
                <!-- ./col -->
              </div>
            </div>
            <!-- /.card-body -->
          </div>
          <!-- /.card -->
          <div class="card">
            <div class="card-body">
              <table id="example2"  class="table table-sm table-bordered" style="display:none">
                <thead>
                  <th>Nombre Paciente</th>
                  <th>Código  Paciente</th>
                  <th>Vacuna</th>
                  <th>Dosis</th>
                  <th class="fechaVacuna">Fecha de vacunación</th>
                </thead>
                <tbody id="tabla-vacunas2">
                  
                </tbody>
              </table>
              <table id="example3"  class="table table-sm table-bordered" style="display:none">
                <thead>
                  <th>Nombre Paciente</th>
                  <th>Código Paciente</th>
                  <th>Vacuna</th>
                  <th>Dosis</th>
                  <th class="fechaVacuna">Fecha de vacunación</th>
                </thead>
                <tbody id="tabla-vacunas3">
                  
                </tbody>
              </table>
              <table id="example4"  class="table table-sm table-bordered" style="display:none; margin: 0 auto;">
                <thead>
                  <th>Nombre Paciente</th>
                  <th>Código Paciente</th>
                  <th>Vacuna</th>
                  <th>Dosis</th>
                  <th class="fechaVacuna">Fecha de vacunación</th>
                </thead>
                <tbody id="tabla-vacunas4">
  
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
      <!-- /.row -->
    </div>
    <input id="usuario" type="hidden" value="<?php echo  $nombre . " " . $primerApellido . " " . $segundoApellido;?>"/>
  </section>
  <!-- /.content -->
</div>

<!-- jQuery -->
<script src="<?php echo base_url(); ?>/adminLte/plugins/jquery/jquery.min.js"></script>
<script src="<?php echo base_url(); ?>/js/custom.js"></script>

<script type="text/javascript">
   var tablaDosis2 = $("#tabla-vacunas2");
   var tablaDosis3 = $("#tabla-vacunas3");
   var tablaDosis4 = $("#tabla-vacunas4");

   var autor = $('#usuario').val();

  $('#reporte-hoy').on('click', function() {
    $("a#reporte-hoy").addClass("selected-card");
    $("a#reporte-pendiente").removeClass("selected-card");
    $("a#reporte-vencido").removeClass("selected-card");


    $("table#example2").removeAttr("style");
    $("#example2_wrapper").removeAttr("style");
    $("#example3_wrapper").attr("style", "display:none");
    $("#example4_wrapper").attr("style", "display:none");


    $.ajax({
      url: '<?= base_url() ?>reportes/reporteHoy',
      type: 'GET',
      dataType: 'json',
      success: function(r) {
        tablaDosis2.find('tr').remove();
        var tituloReporte = "Reporte de " + $('#titulo-hoy').text();
        $(r).each(function(indice, valor) {
          console.log(valor);
          tablaDosis2.append(
            '<tr>' +
            '<td>' + valor.nombrePaciente + " " + valor.primerApellido + " " + valor.segundoApellido + '</td>'+
            '<td>' + valor.codigo + '</td>'+
            '<td>' + valor.nombrevacuna + '</td>'+
            '<td>' + valor.dosis + " " + valor.nombrevia + '</td>'+
            '<td>' + valor.fechaVacuna + '</td>'+
            '</tr>'
            );
        });
        
        $("#example2").DataTable({
          dom: 'Bfrtip',
          "responsive": true,
          "lengthChange": false,
          "autoWidth": false,
          "searching": true,
          "retrieve": true,
          "language": getLanguaje(),
          "buttons": [{
            extend: 'pdfHtml5',
            title: tituloReporte.toUpperCase(),
            bom: true,
            pageSize: "LETTER",
            customize: function ( doc ) {
              doc.content.splice( 0, 0, obtenerImagenCabecera() );
              var today = getDate()
              var objFooter = {};
              objFooter['alignment'] = 'center';
              doc['footer'] = function(currentPage, pageCount) {
                var footer = [
                  {
                    alignment: 'left',
                    stack: [
                      {
                        text: 'Autor: ' + autor,
                        color: 'grey',
                        alignment: 'left',
                        margin:[20, 5, 0, 0]
                      },
                      {
                        text: today,
                        color: 'grey',
                        fontSize: 8,
                        alignment: 'left',
                        margin:[20, 2, 0, 0]
                      }
                    ]
                  },
                  {
                    stack: [
                      {
                        text: 'Pagina ' + currentPage + " de " + pageCount,
                        alignment: 'right',
                        color: 'grey',
                        fontSize: 8,
                        margin:[0, 2, 20, 0]
                      }
                    ]
                  }
                ];
                objFooter['columns'] = footer;
                return objFooter;
              };
            }
          }]
        }).buttons().container().appendTo('#example2_filter .col-sm-6:eq(0)'); 
      },
      error: function() {
        alert('Ocurrio un error en el servidor ..');
      }
    });
      
  });

  $('#reporte-pendiente').on('click', function() {
    $("a#reporte-hoy").removeClass("selected-card");
    $("a#reporte-pendiente").addClass("selected-card");
    $("a#reporte-vencido").removeClass("selected-card");

    $("#example3").removeAttr("style");
    $("#example3_wrapper").removeAttr("style");
    $("#example2_wrapper").attr("style", "display:none");
    $("#example4_wrapper").attr("style", "display:none");

    $.ajax({
      url: '<?= base_url() ?>reportes/reporteVacunasPendientes',
      type: 'GET',
      dataType: 'json',
      success: function(r) {
        tablaDosis3.find('tr').remove();
        var tituloReporte = "Reporte de " + $('#titulo-vacuna-pendiente').text();
        $(r).each(function(indice, valor) {
          tablaDosis3.append(
            '<tr>' +
            '<td>' + valor.nombrePaciente + " " + valor.primerApellido + " " + valor.segundoApellido + '</td>'+
            '<td>' + valor.codigo + '</td>'+
            '<td>' + valor.nombrevacuna + '</td>'+
            '<td>' + valor.dosis + " " + valor.nombrevia + '</td>'+
            '<td>' + valor.fechaSiguienteDosis + '</td>'+
            '</tr>'
            );
        });

        $("#example3").DataTable({
          dom: 'Bfrtip',
          "responsive": true,
          "lengthChange": false,
          "autoWidth": false,
          "searching": true,
          "retrieve": true,
          "language": getLanguaje(),
          "buttons": [{
            extend: 'pdfHtml5',
            title: tituloReporte.toUpperCase(),
            bom: true,
            pageSize: "LETTER",
            customize: function ( doc ) {
              doc.content.splice( 0, 0, obtenerImagenCabecera() );
              var today = getDate()
              var objFooter = {};
              objFooter['alignment'] = 'center';
              doc['footer'] = function(currentPage, pageCount) {
                var footer = [
                  {
                    alignment: 'left',
                    stack: [
                      {
                        text: 'Autor: ' + autor,
                        color: 'grey',
                        alignment: 'left',
                        margin:[20, 5, 0, 0]
                      },
                      {
                        text: today,
                        color: 'grey',
                        fontSize: 8,
                        alignment: 'left',
                        margin:[20, 2, 0, 0]
                      }
                    ]
                  },
                  {
                    stack: [
                      {
                        text: 'Pagina ' + currentPage + " de " + pageCount,
                        alignment: 'right',
                        color: 'grey',
                        fontSize: 8,
                        margin:[0, 2, 20, 0]
                      }
                    ]
                  }
                ];
                objFooter['columns'] = footer;
                return objFooter;
              };
            }
          }]
          
        }).buttons().container().appendTo('#example3_filter .col-sm-6:eq(0)');
        
      },
      error: function() {
        alert('Ocurrio un error en el servidor ..');
      }
    });
  });

  //
  $('#reporte-vencido').on('click', function() {
    $("a#reporte-hoy").removeClass("selected-card");
    $("a#reporte-pendiente").removeClass("selected-card");
    $("a#reporte-vencido").addClass("selected-card");

    $("#example4").removeAttr("style");
    $("#example4_wrapper").removeAttr("style");
    $("#example2_wrapper").attr("style", "display:none");
    $("#example3_wrapper").attr("style", "display:none");
      
    $.ajax({
      url: '<?= base_url() ?>reportes/reporteVacunaRezagado',
      type: 'GET',
      dataType: 'json',
      success: function(r) {
        tablaDosis4.find('tr').remove();
        var tituloReporte = "Reporte de " + $('#titulo-vacuna-rezagado').text();
        $(r).each(function(indice, valor) {
          tablaDosis4.append(
            '<tr>' +
            '<td>' + valor.nombrePaciente + " " + valor.primerApellido + " " + valor.segundoApellido + '</td>'+
            '<td>' + valor.codigo + '</td>'+
            '<td>' + valor.nombrevacuna + '</td>'+
            '<td>' + valor.dosis + " " + valor.nombrevia + '</td>'+
            '<td>' + valor.fechaSiguienteDosis + '</td>'+
            '</tr>'
            );
        });
        
        $("#example4").DataTable({
          dom: 'Bfrtip',
          "responsive": true,
          "lengthChange": false,
          "autoWidth": true,
          "searching": true,
          "retrieve": true,
          "language": getLanguaje(),
          "buttons": [{
            extend: 'pdfHtml5',
            title: tituloReporte.toUpperCase(),
            bom: true,
            pageSize: "LETTER",
            customize: function ( doc ) {
              doc.content.splice( 0, 0, obtenerImagenCabecera() );
              var today = getDate()
              var objFooter = {};
              objFooter['alignment'] = 'center';
              doc['footer'] = function(currentPage, pageCount) {
                var footer = [
                  {
                    alignment: 'left',
                    stack: [
                      {
                        text: 'Autor: ' + autor,
                        color: 'grey',
                        alignment: 'left',
                        margin:[20, 5, 0, 0]
                      },
                      {
                        text: today,
                        color: 'grey',
                        fontSize: 8,
                        alignment: 'left',
                        margin:[20, 2, 0, 0]
                      }
                    ]
                  },
                  {
                    stack: [
                      {
                        text: 'Pagina ' + currentPage + " de " + pageCount,
                        alignment: 'right',
                        color: 'grey',
                        fontSize: 8,
                        margin:[0, 2, 20, 0]
                      }
                    ]
                  }
                ];
                objFooter['columns'] = footer;
                return objFooter;
              };
            }
          }]
        }).buttons().container().appendTo('#example4_filter .col-sm-6:eq(0)'); 
      },
      error: function() {
        alert('Ocurrio un error en el servidor ..');
      }
    });
  });

  function getDate() {
    var today = new Date();
    var dd = today.getDate();
    var mm = today.getMonth() + 1; //January is 0!
    var yyyy = today.getFullYear();
    if(dd < 10){
        dd = '0' + dd
    }
    if(mm < 10){
        mm = '0' + mm
    }
    return dd + '/' + mm + '/' + yyyy;
  }

  function obtenerImagenCabecera() {
    return {
      margin: [ 0, 0, 0, 12 ],
      alignment: 'right',
      width: 200,
      image: 'data:image/png;base64,'
    }
  }
</script>